...
class TestMenuButton(unittest.TestCase):
    ...
    def test_draw(self, mock_get_pressed, mock_get_pos):
        ...
        self.assertFalse(button.draw(surf), "Button fail to load")  # No action expected
        
        # Check if the button image is set correctly when not hovered
        self.assertEqual(button.image.get_width(), button.assets['startbtn'].get_width(), "Button fail to get width")
        self.assertEqual(button.image.get_colorkey(), button.assets['startbtn'].get_colorkey(), "Button fail to load height")
        ...
        self.assertEqual(button.image.get_width(), button.assets['startbtnhover'].get_width(), "Button fail to get width")
        self.assertEqual(button.image.get_colorkey(), button.assets['startbtnhover'].get_colorkey(), "Button fail to load height")
