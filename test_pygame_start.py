    ...
    def test_run(self):
        ...
        with patch('pygame.display.set_mode'):
            ...
            with patch('pygame.transform.scale'):
                ...
                self.assertIsNotNone(self.start.display, "Fail to load display")
                # Assert that 'startbtn' asset is present in the assets dictionary
                self.assertTrue('startbtn' in self.start.assets, "Fail to load Buttons")
                # Assert that 'background' asset is present in the assets dictionary
                self.assertTrue('background' in self.start.assets, "Fail to load background")
                # Assert that 'bg_clouds' asset is present in the assets dictionary
                self.assertTrue('bg_clouds' in self.start.assets, "Fail to load background clouds")
                # Assert that the bg_clouds attribute is not None after running
                self.assertIsNotNone(self.start.bg_clouds, "Fail to check cloud status")
                # Assert that the startbtn attribute is not None after running
                self.assertIsNotNone(self.start.startbtn, "Fail to chech button status")
